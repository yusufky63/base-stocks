 ...
 */
export function PlanExecutor({ allocations, source, disabled, onComplete }: Props) {
  ...
  const insufficient = isConnected && totalUsd > usdcBalanceUsd;
    ...
    <div className="flex flex-col gap-4">
      {!execution && (
        <>
          ...
          {insufficient && <p className="text-[13px] text-danger-fg">You don’t have enough USDC for this amount.</p>}
          {planErrors.map((e) => (
          ...
          {plan && (
            <div className="flex gap-2">
              ...
              {!isConnected ? (
                ...
                <Button full size="lg" disabled={insufficient || plan.legs.length === 0 || plan.legs.every((l) => l.quoteError)} onClick={() => exec.start(plan)}>
                  Buy {plan.legs.length} {plan.legs.length === 1 ? "stock" : "stocks"} for {formatUsd(plan.totalUsd - plan.keepUsdcUsd)}
                </Button>
              )}
            </div>
          )}
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="border border-line rounded-[8px] overflow-hidden">
        ...
        {plan.legs.map((l) => (
          <div key={l.assetAddress} className="grid grid-cols-[1fr_70px_90px_1fr] px-3 py-2 border-b border-line last:border-b-0 text-[14px] items-center">
            <span className="font-medium">{l.symbol.replace(/c$/, "")}</span>
            ...
            <span className="text-right font-mono num text-[13px]">{l.quoteError ? <span className="text-danger-fg">{l.quoteError}</span> : l.estimatedBuyAmount ? formatTokenAmount(l.estimatedBuyAmount, 8) : "—"}</span>
          </div>
        ))}
        {deferred.map((d) => (
          <div key={`deferred-${d.assetAddress}`} className="grid grid-cols-[1fr_70px_90px_1fr] px-3 py-2 border-b border-line last:border-b-0 text-[14px] items-center bg-surface/60">
            <span className="font-medium text-ink-secondary">
              {d.symbol.replace(/c$/, "")} <span className="ml-1 font-mono text-[10px] uppercase tracking-[0.08em] text-warning-fg">not issued</span>
            </span>
      ...
      {deferred.length > 0 && (
        <div className="flex items-center justify-between gap-2 flex-wrap text-[13px]">
          <span className="text-ink-secondary">Money for stocks not issued yet</span>
      ...
      <KeyValue k="Execution" v={`${plan.legs.length} separate purchases, confirmed one by one`} mono={false} />
      {plan.warnings.map((w) => (
