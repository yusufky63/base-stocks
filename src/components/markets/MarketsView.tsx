...
import { isLiveMarket, liveMarketTotals, marketLegend, type MarketRow as Row } from "./live-markets";
...
/** Built once: the labels come from `tradingStatus()` itself, so this cannot describe a status the chips no longer show. */
...
/**
 * One stock. The whole row opens the stock page: the name link is stretched over the row with a
 * pseudo-element, and the buttons that sit on top of it (watch, Buy) are raised above that layer,
 * so the markup stays one link plus separate controls rather than controls nested inside a link.
 */
function MarketRow({ asset, price, spark, watched, onToggleWatch, restricted }: { asset: AssetsResponse["assets"][number]; price?: AssetsResponse["prices"][string]; spark?: number[]; watched: boolean; onToggleWatch?: () => void; restricted: boolean }) {
  ...
  const tradable = isLiveMarket(view.status);
  const muted = view.status === "not-issued" || view.status === "paused";
  const canBuy = tradable && !restricted;
  const watchButton = (size: string) =>
    onToggleWatch ? (
      <button type="button" aria-label={watched ? "Remove from watchlist" : "Add to watchlist"} aria-pressed={watched} onClick={onToggleWatch} className={cx("relative z-10 inline-flex items-center justify-center rounded-[6px] border transition-fast", size, watched ? "text-primary border-primary bg-primary-soft" : "text-ink-muted border-transparent hover:border-line hover:text-ink")}>
    ...
    <div className={cx("rail relative grid grid-cols-[1fr_auto] md:grid-cols-[1fr_96px_130px_96px_130px_120px_150px] items-center px-4 py-3 border-b border-line last:border-b-0 gap-3 hover:bg-surface transition-fast", muted && "opacity-75 hover:opacity-100")}>
      ...
      <div className="md:hidden text-right flex flex-col items-end gap-1.5">
        ...
        <div className="flex items-center gap-1.5">
          {watchButton("h-9 w-9")}
          {canBuy && (
            <Link href={`/stocks/${asset.address}?trade=buy`} className="relative z-10 inline-flex items-center justify-center gap-1 h-9 px-3 rounded-[6px] text-[13px] font-medium bg-primary-fill text-primary-contrast hover:brightness-[1.08] transition-fast">
      ...
      <div className="hidden md:flex items-center justify-end gap-1.5">
        ...
        {canBuy ? (
          ...
          <Link href={`/stocks/${asset.address}`} title={restricted && tradable ? "Trading is not available in your region" : undefined} className="relative z-10 inline-flex items-center justify-center h-9 min-w-[88px] px-3 rounded-[6px] text-[13px] font-medium border border-line text-ink-secondary hover:text-ink hover:border-line-strong transition-fast">
...
function MarketStats({ rows }: { rows: Row[] }) {
  const t = liveMarketTotals(rows);
  const cells = [
    { label: "Live markets", value: t.known ? `${t.live.length} of ${rows.length}` : "—" },
    { label: "DEX liquidity", value: t.liquidityUsd > 0 ? formatUsdCompact(t.liquidityUsd) : "—" },
    { label: "24h volume", value: t.volume24hUsd > 0 ? formatUsdCompact(t.volume24hUsd) : "—" },
    { label: "Up today", value: t.live.length ? `${t.up} / ${t.live.length}` : "—" },
  ];
  return (
