...
export function PoolsModule({ address }: { address: Address }) {
  const { data, isLoading, isError } = useQuery({ queryKey: ["market", "pools", address.toLowerCase()], queryFn: () => apiGet<PoolMapResponse>(`/api/market/${address}/pools`), staleTime: 2 * 60_000 });
  ...
  return (
    <div className="flex flex-col gap-2">
          ...
          {data.pools.map((p) => (
            <li key={p.address} className={cx("grid grid-cols-[1fr_auto] sm:grid-cols-[1fr_90px_90px_auto] gap-x-3 gap-y-1 px-3 py-2 items-center", !p.known && "opacity-70")}>
              ...
              <span className="text-right font-mono num text-[12px] sm:contents">
                <span className="block sm:text-right">{formatUsdCompact(p.reserveUsd)}</span>
                <span className="block sm:text-right text-ink-secondary">{p.volume24hUsd !== null ? formatUsdCompact(p.volume24hUsd) : "—"}</span>
              </span>
      ...
      <p className="text-[11px] text-ink-muted">Reserves from GeckoTerminal (checked against onchain balances within 0.3%). &ldquo;Routed&rdquo; means the trade comparison can reach the pool through KyberSwap, Velora or the Uniswap API; &ldquo;LP tracked&rdquo; means positions in it appear under Earn and on this page.</p>
    </div>
  );
}
