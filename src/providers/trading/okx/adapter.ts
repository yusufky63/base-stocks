 ...
 * Docs: https://web3.okx.com/build/dev-docs/dex-api/dex-swap-api  (quote · swap · approve-transaction)
 */
  ...
  .object({
    ...
    subRouterList: z
      .array(
        z
          .object({
            dexProtocol: z.array(z.object({ dexName: z.string().optional(), percent: numLike.optional() }).passthrough()).optional(),
          })
          .passthrough(),
      )
      .optional(),
...
const quoteSchema = z
  .object({
    ...
    priceImpactPercentage: numLike.optional(),
...
function fills(q: z.infer<typeof quoteSchema>): Array<{ source: string; proportionBps: number | null }> {
  ...
  for (const r of q.dexRouterList ?? []) {
    const names = (r.subRouterList ?? []).flatMap((s) => (s.dexProtocol ?? []).map((d) => d.dexName ?? "unknown"));
    const pct = r.routerPercent ? Math.round(Number(r.routerPercent) * 100) : null;
...
function baseQuery(intent: TradeIntent): Record<string, string> {
  return { chainIndex: BASE_CHAIN_INDEX, chainId: BASE_CHAIN_INDEX, fromTokenAddress: intent.sellToken, toTokenAddress: intent.buyToken, amount: intent.sellAmount.toString() };
}

function normalize(q: z.infer<typeof quoteSchema>, intent: TradeIntent, spender: Address | null): IndicativeQuote {
  const buy = toBig(q.toTokenAmount) ?? 0n;
  const feeWei = toBig(q.estimateGasFee);
  return {
    provider: "okx",
...
async function spenderFor(intent: TradeIntent): Promise<Address | null> {
  ...
  const data = await okxRequest<unknown[]>("GET", "/api/v5/dex/aggregator/approve-transaction", { query: { chainIndex: BASE_CHAIN_INDEX, chainId: BASE_CHAIN_INDEX, tokenContractAddress: intent.sellToken, approveAmount: intent.sellAmount.toString() } });
...
export class OkxTradeProvider implements TradeProvider {
  ...
  async getIndicativeQuote(intent: TradeIntent): Promise<IndicativeQuote> {
    const data = await okxRequest<unknown[]>("GET", "/api/v5/dex/aggregator/quote", { query: baseQuery(intent) });
  ...
  async getExecutableQuote(intent: TradeIntent): Promise<ExecutableQuote> {
    ...
    const [data, spender] = await Promise.all([
      okxRequest<unknown[]>("GET", "/api/v5/dex/aggregator/swap", { query: { ...baseQuery(intent), slippage: (intent.slippageBps / 10_000).toString(), userWalletAddress: intent.taker, swapReceiverAddress: intent.recipient ?? intent.taker } }),
    ...
    const base = normalize(parsed.data.routerResult ?? {}, intent, spender ?? (parsed.data.tx.to as Address));
