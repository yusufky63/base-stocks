 ...
 *   x           → the ecosystem's own accounts on X, through the public embed feed (no API key)
...
export interface NewsItem {
  ...
  /** Unix ms. */
  publishedAt: number;
  /** Ticker the headline was fetched for; "MARKETS" for market-wide feeds, "BASE" for the ecosystem feed, "X" for posts. */
  ticker: string;
  via: NewsVia;
...
async function fetchText(url: string, timeoutMs: number, accept = "application/rss+xml, application/xml, text/xml"): Promise<string> {
