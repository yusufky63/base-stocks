...
export const POST = route({ rateLimit: { key: "watchlist.write", limit: 60, windowMs: 60_000 } }, async (req) => {
...
export const DELETE = route({ rateLimit: { key: "watchlist.write", limit: 60, windowMs: 60_000 } }, async (req) => {
  const { owner, assetAddress } = await parseBody(req, bodySchema);
