...
const STEPS = [
  ...
  { n: "02", title: "Understand", body: "You see one price and trade at it: the pool's. The stock's own price (the issuer's Chainlink feed) stays in the background — the Reference column on Markets, and a warning wherever a pool strays far from it. Executable is what a live quote gives you for your exact amount." },
...
const FAQ: Array<{ q: string; a: string }> = [
  ...
  { q: "Who executes my trade?", a: "Every quote is requested from several routes at once: KyberSwap, Velora, the Uniswap Trading API, Aerodrome directly, and 0x or OKX when they are enabled. BaseStocks shows every answer, picks the best net output by default, and lets you choose a provider. The swap itself is a transaction your wallet signs." },
  ...
  { q: "What fees do I pay?", a: "The Base network fee (usually cents), any DEX fee inside the quoted price, and any provider fee shown in the quote. BaseStocks charges nothing itself. Base Account users may get sponsored gas when a paymaster is configured." },
  { q: "Is BaseStocks custodial?", a: "No. Tokens stay in your wallet, approvals are limited to the exact amount and spender, and nothing is signed on page load. An automatic plan draws on a USDC allowance you set and can revoke; the contract never holds your stock. BaseStocks stores records (trades, gifts, plans, profiles) offchain to explain what happened, never keys or funds." },
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="border border-line rounded-[8px] bg-canvas p-5 flex flex-col gap-3">
          ...
          <ul className="text-[14px] flex flex-col gap-2 list-disc pl-5">
            <li>Self-custodial: BaseStocks never holds keys or funds and never asks for a signature on page load.</li>
        ...
        <div className="border border-line rounded-[8px] bg-canvas p-5 flex flex-col gap-3">
          ...
          <ul className="text-[14px] flex flex-col gap-2 list-disc pl-5">
            <li>You pay the network fee on Base (typically cents) and any DEX fee inside the quoted price. BaseStocks shows the estimated fee before you confirm.</li>
