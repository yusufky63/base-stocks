 ...
 * SDK only touches these names inside its Solana provider methods, which nothing in BaseStocks
 * calls. If one ever were called, it fails loudly rather than silently.
 */
function unsupported(): never {
  throw new Error("Solana is not supported in BaseStocks; every action settles on Base.");
}
