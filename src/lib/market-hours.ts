...
/**
 * When the regular session next opens or closes, walked forward in five-minute steps (at most a
 * long weekend). Holidays are not modelled, the same limit as `isUsMarketOpen`; on one, "opens
 * in" is early by a day and the feed's own freshness says the rest.
 */
export function nextSessionBoundary(at: Date = new Date()): SessionBoundary {
  const open = isUsMarketOpen(at);
  const STEP = 5 * 60_000;
  // Snap to the next five-minute mark so the walk lands on :30 and :00 exactly.
