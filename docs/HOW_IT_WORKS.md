...
| Public profile | `/u/[handle-or-address]` | Allocation in percent (if public), badges, published baskets. |
...
- **Warm-up.** `src/instrumentation.ts` refreshes assets/prices at boot, runs B20 discovery (boot: 450k blocks, then every 30 min: 60k), and probes 0x hourly — on long-running Node. On Vercel (`VERCEL` set) it only loads the stored registry and validates the environment once (a malformed optional variable is logged and ignored rather than failing every route); `vercel.json` schedules `/api/cron/refresh?job=discovery` and `?job=sweep` (Bearer `CRON_SECRET`) once a day, and Supabase `pg_cron` carries the fifteen-minute jobs (see Schedules below).
- **Proxy.** `src/proxy.ts` enforces the geoblock on execution routes (`/api/trade/*`, `/api/earn/prepare`, `/api/portfolio/(plan|intent)`, `/api/automation/prepare-run`) and on writes to gifts and gift pools; the same handlers also call `assertTradingAllowed` themselves. There is no referral capture.
...
Price model (`src/services/price-service.ts`): `displayUsd` is the DEX market price when a pool exists, that pool holds at least $20k, **and** the price is within 20% of the reference while the feed is live; otherwise the Chainlink reference (marked *reference*). The deviation check is mandatory rather than one of several ways to pass, because depth and correctness are different properties: TSLAc was displayed at $17,936 against a $366 reference off a pool holding $717k. When the feed is stale or frozen there is nothing to check against and the market price takes over, which is the ordinary off-hours state. `executablePriceUsd` comes from the provider quote; `executablePriceUsd` comes from the provider quote; price impact is measured against the reference or market basis. Sparklines are 7-day closes. The status model (`src/lib/trading-status.ts`) turns liquidity into Live (≥ $100k), Thin ($10k–100k), Very thin (a pool under $10k — labelled with a slippage warning rather than "No pool yet", which would contradict the size printed beside it), No pool (nothing at all), Not issued (zero supply), Paused. Only Live and Thin count towards "live markets" in the hero and market totals, so a $108 pool never inflates them.
...
| GiftEscrow (ours) | V2 `0x59E4C2C5AfbDae22A09566EB69f283c7f884EB23` on Base (deployed 2026-09-13: zero-recipient check, single-use claim keys); V1 `0x8D9fE4b3Ab9BecbE1181d15d51FB9724561C7f55` keeps paying out the gifts created in it (records carry `escrowAddress`) | none | Claim-link gifts (`contracts/src/GiftEscrow.sol`, 17 Foundry tests). Ownerless: only pays the claim-signed recipient or refunds the sender. The link fragment carries an ephemeral key; its address hashes to the onchain gift id, EIP-712 `Claim(giftId,recipient)` binds the recipient, anyone may submit (sponsored claims). Expiry ≤ 90 days; sender can cancel any time. Routes: `/gifts/claim/[id]` + share card. |
| AutoInvest (ours) | V2 `0x708e63F591E4983b2B233C71EA1283ca98e4C97a` on Base (deployed 2026-09-13: reference floor on the total-return feed, `FloorUnavailable` when a registered feed is stale); V1 `0xc767844F2D65ba241DBe2c04f9c01d05cCD9b60E` (2026-09-05) still runs the plans created in it; both source verified on Basescan; owner `0x78de…87FF`, keeper `0xffA7…c39C`; allow-list KyberSwap, Aerodrome, OKX routers on both — see [AUTO_INVEST.md](AUTO_INVEST.md) | keeper key on the server | Recurring purchases that run without the owner (`contracts/src/AutoInvest.sol`, 31 Foundry tests). A plan = legs + weights + amount per run + cadence + expiry + slippage tolerance; `execute` pulls one run's USDC under a normal allowance, swaps through an allow-listed router with the owner as recipient, checks the owner's balance grew by at least `max(minOut, Chainlink floor)`, refunds leftovers. Amount, cadence, routes (24 h announcement for additions) and output are enforced onchain; the keeper only chooses timing and route. |

### 5.2 Flow

1. **Indicative price** (`POST /api/trade/price`): `b20Guard.preTradeCheck` (canonical asset, pause, policy, region), amount validation, then `compareProviders` asks every configured provider at once (2.5 s budget each), values each answer in USD at the current price minus the estimated network fee, and returns the best plus every alternative (`TradeQuoteAlternative`). The panel renders a route picker; auto = best net, manual = that provider only (`strictProvider`). **Best execution** (`bestExecution: true`, a per-device switch in the panel) prefers CoW Protocol's batch auction whenever its quote is within `BEST_EXECUTION_TOLERANCE_PCT` (0.5%) of the best net swap: solvers compete for the order, the winner pays the gas and a signed order cannot be sandwiched. The answer carries `execution` (`mode`, `applied`, `suggested`, `note`); above `BEST_EXECUTION_MIN_USD` ($250) the panel suggests the switch when CoW is competitive. The firm quote asks CoW first under the switch and the swap chain covers a miss.
...
- **Community**: published baskets (`baskets`, `basket_votes`), 7-day pulse from trade records, clones; profiles with handle, display name, bio, visibility; badges with server-computed progress (`computeBadges`). Votes and clones are one per signed-in wallet (`basket_votes`, `basket_clones`) and the counters move by atomic SQL increments. There is no referral programme.
...
`trade_records`, `gifts`, `portfolio_executions` + `portfolio_execution_steps`, `portfolio_templates` + `portfolio_template_allocations`, `portfolio_snapshots` (daily value history), `watchlists`, `profiles`, `baskets` + `basket_votes` + `basket_clones`, `automation_rules`, `earn_actions`, `discovered_assets` (with `underlying`, `chainlink_feed`, `tags`, `eligible`, `auto_verified`, `creator`, `reason`), `ai_usage`, `ai_digests`, `tx_receipts` (verified receipts: hash, status, block, block time — written once, shared by every instance), `sync_cursors` (where each chain sweep left off), `kv_cache` (the shared cache tier), `wallet_index` + `chain_transfers` (the app's own index of tokenized-stock transfers for the wallets it knows), `error_events` (one counted row per distinct error). `trade_records`, `gifts`, `earn_actions` and `gift_pools` carry `verified_at` / `verify_note`. Schema: `supabase/schema.sql` is the readable reference; the migrations applied through the Supabase MCP (and the files under `supabase/migrations/`) are the source of truth.
...
| `GET /api/profiles/[ref]`, `/me`, `GET/POST /api/baskets`, `/api/community/pulse`, `/api/watchlist` | Community and profile |
...
- **Schedules.** Supabase `pg_cron` + `pg_net` (migration `pg_cron_app_schedules`, 2026-09-13) tick the AutoInvest keeper every 15 minutes and call the time-sensitive cron jobs (`index`, `verify`, `earn`, `pools`, `rollup`, `stats`, `status`) once each per quarter hour, offset a minute apart; the bearer is read from Vault under the name `cron_secret` (stored 2026-09-13 with `node scripts/vault-cron-secret.mjs`, which reads `CRON_SECRET` from `.env` and never prints it; rerun it after rotating the secret). `.github/workflows/refresh.yml` and `keeper.yml` ask for the same cadence but GitHub's schedule ran them about seven times a day; they are switched off with the repo variables `REFRESH_ENABLED=false` / `KEEPER_ENABLED=false` and can be turned back on as a fallback. Overlapping ticks are safe either way because a plan is claimed in one statement. `vercel.json` keeps two daily runs, `?job=discovery` and `?job=sweep` (the Hobby plan allows two crons; the automation entry went, pg_cron ticks the keeper); the sweep also prunes `error_events` older than 14 days and the daily AI counters older than 7. `monitor.yml` reads `/api/health` every 30 minutes and fails the run — GitHub emails the owner — when `alerts` is not empty: keeper below 0.0005 ETH, a dependency down, 20+ errors in the last hour, a storage table missing.
